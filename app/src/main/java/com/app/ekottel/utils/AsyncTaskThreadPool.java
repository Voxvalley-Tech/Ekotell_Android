...
public class AsyncTaskThreadPool {
	// classes used for the threadPool were created because of an issue with
	// customized parameters of
	// ThreadPoolExecutor:
	...
	private static final int MAXIMUM_POOL_SIZE = Math.max(CORE_POOL_SIZE,
	...
	private final Set<AsyncTaskEx<?, ?, ?>> mTasks = new HashSet<AsyncTaskEx<?, ?, ?>>();

	public AsyncTaskThreadPool() {
		this(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, TIME_TO_KEEP_ALIVE_IN_SECONDS);
	}

	public AsyncTaskThreadPool(final int minNumberOfThread,
			final int maxNumberOfThread, final int keepAliveTimeInSeconds) {
		final ScalingQueue<Runnable> poolWorkQueue = new ScalingQueue<Runnable>();
		final ThreadFactory threadFactory = new ThreadFactory() {
			private final AtomicInteger mCount = new AtomicInteger(1);

			@Override
			public Thread newThread(final Runnable r) {
				final Thread thread = new Thread(r, "thread #"
		...
		mExecutor = new ScalingThreadPoolExecutor(minNumberOfThread,
				maxNumberOfThread, keepAliveTimeInSeconds, TimeUnit.SECONDS,
	...
	public <Params, Progress, Result> void executeAsyncTask(
			final AsyncTaskEx<Params, Progress, Result> task,
			@SuppressWarnings("unchecked") final Params... params) {
